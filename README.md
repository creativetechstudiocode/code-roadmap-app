...
A companion learning site for [Creative Tech Studio](https://creativetechstudio.org). Code's Roadmap teaches people to write code and work with real developer tools aimed at anyone actually building and shipping things.
...
A matched pair of starter files lives in `assets/`. Copy one into `pages/`, edit the `EDIT:`-marked spots, and fill in its content array and the layout, styling, and behavior come pre-wired.
...
**Which one?** The quick test: if someone would start at the top and work down once, it's a **roadmap**; if they'd come back repeatedly to look up one thing, it's a **guide**. Pick by the shape of the content, not the topic because the same subject could be either.

**`assets/design-system.html`** is the living design doc and you can open it in a browser. It documents the palette, type scale, spacing, components, layout patterns for all three page shapes, voice/copy rules, do/don'ts, and a "Building a New Page" section that walks through choosing a template with a checklist for each.
...
6. Add a linking card on `index.html` in the right category grid and set `data-diff` (`beginner` / `intermediate`) for a roadmap, or tag it **Guide** for a guide. Match the card title to the page title word for word.
